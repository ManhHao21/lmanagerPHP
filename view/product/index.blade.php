                ...
                <div class="row">
                    <div class="card w-100">
                        <div class="card-header flex">
                            ...
                            <a href="/admin/product/create" class="btn btn-primary flex justify-content-end">Thêm sản
                                phẩm</a>
                        </div>
                                    ...
                                    <tbody>
                                        @foreach ($products as $index => $product)
                                            <tr>
                                                <td>{{ $index + 1 }}</td>
                                                <td>
                                                    <img src="/uploads/{{ urlencode($product['image']) }}"
                                                        alt="Hình ảnh sản phẩm" class="img-thumbnail" width="80"
                                                        height="80">
                                                </td>
                                                <td>{{ $product['name'] }}</td>
                                                ...
                                                <td>
                                                    @if (!empty($product['PromotionPrice']))
                                                        {{ number_format($product['PromotionPrice'], 0, ',', '.') }} VND
                                                    @else
                                                        -
                                                    @endif
                                                </td>
                                                ...
                                                <td>{!! $product['Hot'] ? '<span class="badge bg-success">Hot</span>' : '-' !!}</td>
                                                <td>{{ $product['slug'] ?? '-' }}</td>
                                                <td>
                                        ...
                                        @if (count($products) == 0)
                                            <tr>
                                                <td colspan="16" class="text-center">Không có sản phẩm nào.</td>
                                            </tr>
                                        @endif
                                    </tbody>
